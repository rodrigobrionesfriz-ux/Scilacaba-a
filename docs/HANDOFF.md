...
**Última actualización**: 2026-06-28 por sesión Fase 6b (Claude)
**Rama activa**: `feat/fase-6b-cuaderno` (desde `develop`). Pendiente: PR a `develop` + verificación e2e/manual por el usuario. Próxima sesión: crear `feat/fase-6c-cuaderno` (Fertirriego) desde `develop`.
...
| 6c   | Cuaderno: fertirriego (sectores/OAF/config/aportes) | PENDIENTE | — | — |
| —    | Cuaderno: estimación de cosecha → movida a Fase 7 (depende de terreno offline) | PENDIENTE | — | — |
...
8. **Fase 6c** (Cuaderno: Fertirriego — sectores, órdenes OAF, config singleton, aportes nutricionales) — **SIGUIENTE**: dominio aparte (`fertirriego_*`, correlativo `OAF`); reusar la infra de 6a/6b. La **Estimación de cosecha** se difiere a Fase 7 (depende de `conteos`/`invplantas`, datos de terreno offline). Reportes Excel → Fase 10.
