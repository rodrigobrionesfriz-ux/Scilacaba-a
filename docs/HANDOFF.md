...
| 6a   | Cuaderno: paños + catálogo + aplicaciones | HECHO | feat/fase-6a-cuaderno | typecheck/lint/83 tests/build verdes; e2e por el usuario pendiente |
...
6. ~~**Fase 6a** (Cuaderno: paños + catálogo + aplicaciones)~~ — **HECHO** en `feat/fase-6a-cuaderno`. Falta: PR a `develop` + verificación e2e/manual por el usuario (login admin/agronomo → `/cuaderno`, CRUD de las 3 pestañas; rol `gerente` solo lectura).
7. **Fase 6b/6c** (Cuaderno: Órdenes OA + distribución, Confirmaciones, Fertirriego, Estimación): reusar la infra de 6a; las OA calculan distribución por paño (agua/producto) y las confirmaciones recalculan por agua real. Reportes Excel → Fase 10.
