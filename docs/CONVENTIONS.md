...
11. **Nunca `any` ni cast con `as`** — tipar correctamente (genéricos, `unknown` + narrowing, o inferencia de zod).
...
## Mapa de carpetas (capas técnicas bajo `src/`)
...
src/
  ...
  lib/utils.ts                   # utilidades comunes (cn, etc.)       (regla 8)
